...
def test_operation_result_is_frozen() -> None:
    r = OperationResult(
        action="install", plugin_id="com.x", success=True, message="done",
        duration_s=1.2, tracker_id="t1", update_set=None, rollback_version=None,
    )
...
def test_operation_result_rejects_unknown_action() -> None:
    with pytest.raises(ValidationError):
        OperationResult(
            action="reboot", plugin_id="com.x", success=True, message="",
            duration_s=0.0, tracker_id="t", update_set=None, rollback_version=None,
        )


def test_operation_log_is_tuple_and_filterable() -> None:
    r1 = OperationResult(action="install", plugin_id="a", success=True, message="",
        duration_s=0.0, tracker_id="t1", update_set=None, rollback_version=None)
    r2 = OperationResult(action="activate", plugin_id="a", success=False, message="boom",
        duration_s=0.0, tracker_id="t2", update_set=None, rollback_version=None)
...
def test_progress_state_terminal_classification() -> None:
    ps_running = ProgressState.from_sn({"status": "1", "status_label": "In Progress",
        "percent_complete": 50, "error": "", "update_set": None, "rollback_version": None,
        "trackerId": "t", "status_message": "", "status_detail": ""})
    ps_done = ProgressState.from_sn({"status": "2", "status_label": "Success",
        "percent_complete": 100, "error": "", "update_set": None, "rollback_version": "1.0",
        "trackerId": "t", "status_message": "", "status_detail": ""})
    ps_failed = ProgressState.from_sn({"status": "3", "status_label": "Failed",
        "percent_complete": 80, "error": "boom", "update_set": None, "rollback_version": None,
        "trackerId": "t", "status_message": "", "status_detail": ""})
...
def test_dependency_entry_keeps_servicenow_field_names() -> None:
    d = DependencyEntry(
        id="Vulnerability Response Common", orig_string="sn_vul_cmn:2.16.1",
        type="Application", min_version="2.16.1", source_app_id="abc123",
        installed=True, active=True, hide_on_ui=False,
        status="Will be Updated", status_value="will_be_updated",
        order=2, link="nav_to.do?...", has_license=False, is_allowed_install=True,
    )
...
def test_dependency_entry_from_sn_handles_id_casing() -> None:
    ...
    raw = {
        "Id": "Security Incident Response", "orig_string": "sn_si:13.9.23",
        "type": "Application", "minVersion": "13.9.23", "source_app_id": "abc",
        "installed": True, "active": True, "hide_on_ui": False,
        "status": "Installed", "status_value": "installed", "order": 2,
        "link": "x", "has_license": False, "is_allowed_install": True,
    }
