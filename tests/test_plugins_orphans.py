...
def _plugin(
    ...
    *,
    ...
    """Build a minimal PluginInfo for orphan tests.

    Args:
        plugin_id: SN plugin identifier.
        depends_on: Direct dependency plugin IDs.
        state: ``active`` or ``inactive``.
        record_counts: Per-table record counts; ``None`` means uncaptured.

    Returns:
        A frozen PluginInfo populated with test defaults.
    """
...
def test_orphan_candidates_returns_plugin_with_zero_deps_and_zero_records() -> None:
    inv = _inventory(_plugin("com.lonely", record_counts=()))
...
def test_orphan_candidates_excludes_plugin_with_dependents() -> None:
    inv = _inventory(
        _plugin("com.target", record_counts=()),
        _plugin("com.consumer", depends_on=("com.target",), record_counts=()),
    )
    result = orphan_candidates(inv)
...
def test_orphan_candidates_excludes_plugin_with_record_counts_none() -> None:
    inv = _inventory(_plugin("com.unknown"))
    assert orphan_candidates(inv) == ()


def test_orphan_candidates_includes_inactive_plugins() -> None:
    inv = _inventory(
        _plugin("com.dead", state="inactive", record_counts=()),
    )
...
def test_orphan_candidates_excludes_plugin_in_its_own_depends_on() -> None:
    inv = _inventory(
        _plugin("com.loop", depends_on=("com.loop",), record_counts=()),
    )
    assert orphan_candidates(inv) == ()
...
def test_orphan_candidates_sorts_by_state_then_plugin_id() -> None:
    inv = _inventory(
        _plugin("com.b", state="inactive", record_counts=()),
        _plugin("com.a", state="inactive", record_counts=()),
        _plugin("com.z", state="active", record_counts=()),
    )
    result = orphan_candidates(inv)
